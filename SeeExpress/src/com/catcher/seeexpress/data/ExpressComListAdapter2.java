...
import com.catcher.seeexpress.util.Utils;
import com.catcher.seeexpress.view.PinnedHeaderListView.PinnedHeaderAdapter;

public class ExpressComListAdapter2 extends BaseAdapter implements
		PinnedHeaderAdapter, SectionIndexer {

	private Context mContext;
	private List<ExpressCom> mList;

	public ExpressComListAdapter2(Context context, List<ExpressCom> list) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(mContext).inflate(
					R.layout.ckd_phone_item, null);
			holder = new ItemHolder();
			holder.telImageView = (ImageView) convertView.findViewById(R.id.tel_icon);
			holder.groupTextView = (TextView) convertView
					.findViewById(R.id.group_title);
			holder.cityTextView = (TextView) convertView
					.findViewById(R.id.ming);
			holder.phoneTextView = (TextView) convertView.findViewById(R.id.tel);
		...
		try {
			holder.telImageView.setImageResource(Utils.getIdByImageName(ec.getIconPath()));
		} catch (Exception e) {
			e.printStackTrace();
		}
		holder.phoneTextView.setText(ec.getPhone());
